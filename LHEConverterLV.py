...
import sys, argparse
from ROOT import std,TTree,TFile,TLorentzVector
        ...
        for line in input_file:
            ...
            if is_event and is_meta:
                ...
                if len(data) != 13:
                    ...
                    p4 = TLorentzVector()
                    p4.SetXYZM(data[9],data[6],data[7],data[8])
                    m_lv.push_back(p4)
...
def lhe2ntuple(input_path = None, output_path = None):
    #Check for inputs
    if input_path == None:
        print("Error: NO INPUT FILE")
        return
    #If no output file name is given, use the input file as a default, converted to .root format
    out_f_name = output_path
    if out_f_name == None:
        out_f_name = input_path.split('.lhe')[0] + '.root'
